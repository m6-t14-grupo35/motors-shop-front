...
const Header = (isLoggedIn: boolean, user: IUser) => {
  ...
  return (
    <>
      <header
        id="header"
        className="flex-column sm:flex-row fixed top-0 left-0 h-20 z-10 bg-white w-screen flex align-center justify-between border-b-2 grey-6 px-16 flex-wrap"
      >
        ...
        <div className="flex items-center flex-column sm:flex-row flex-wrap">
          <div
            ...
            onClick={() => {
              ...
              toggle
                ? nav?.classList.remove("hidden", "border-l-2", "grey-6") &&
                  nav?.classList.add("flex", "w-screen")
                : nav?.classList.add("hidden", "border-l-2", "grey-6") &&
                  nav?.classList.remove("flex", "w-screen");
            }}
            className="
          sm:hidden
          flex
        ...
        >
          ...
          <button className="w-full body-1-600 border-grey-0 rounded border h-12 py-3 px-7 text-grey-0">
            Cadastrar
          </button>
        </div>
      </header>
